...
class CMakeBuild(build_ext):
    ...
    def build_extension(self, ext):
        ...
        for fn in os.listdir(build_temp_dir):
            print(fn)
            if 'cytnxpath.tmp' in fn:
                self.copy_file(os.path.join(build_temp_dir,fn),extdir)
                print("[Relocate cytnxpath.tmp]: ",fn)
                break
...
setup(
    name='torcyx',
    version='0.1',
    maintainer='Kai-Hsin Wu',
    maintainer_email="kaihsinwu@gmail.com",
    description='Project Torcyx',
    long_description="""This package provides torcyx: Cytnx extension with ML capability base on pytorch """,
    packages=["torcyx"],
    include_package_data=True,
    ext_modules=[CMakeExtension('torcyx')],
